...
class TrendFilterService:
    ...
    async def filter_trends_for_campaign(
        ...
        if company_values is None:
            company_values = ["family-friendly", "positive", "inclusive", "safe"]
        ...
        try:
            ...
            Your task is to filter out trends that are:
            1. Related to negative events (attacks, disasters, tragedies)
            2. Involving specific individuals in negative contexts
            3. Non-family-friendly (gambling, adult content, violence)
            4. Controversial or divisive topics
            5. Not aligned with positive brand values
            ...
            logger.info(f"Filtered trends: {len(trends)} -> {len(filtered_trends)} suitable trends")
